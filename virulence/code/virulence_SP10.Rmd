...
I want to commpare SP10 WT to the $\Delta g120$ on both the WT host ($\Delta 6$) anf its $\Delta 6\Delta sigF$ derivative.

I ran the experiments twice. In the first run I made a mistake in the columns to which the phages were added. This resulted in having complete data from that experiment only for SP10 $\Delta g120$. Below I analyze the data from both experiments.  

## load and organize data
colonies of first experiment labeled #1 and #2. From second experiment labeled #3 and #4
...
Overall, there is a good dilution dependence in the timing of bacterial growth reduction (i.e. lysis). There appear to be a contaminated blank control, and perhaps some wells that did not receive phage in the highest phage dilutions. This is expected.
...
First I will look at the change in OD over time. Storms et al. recommend *establishing the limit of integration as the time at which the slope of OD630 over time reaches ≤0.03 h*  
```{r}
...
This method show that the integrating limit should be at ~ 5 hours. Taking both together I will set integration limit at 6 hours. How does that look? 
...
d.plot %>% 
  ...
  geom_line(size=1)+
  #highlight no phage
   geom_line(data = filter(d.plot, moi=="0" & Time <= 10), size=2)+
  facet_wrap(~ phage)+
  theme_classic()+
  panel_border(color = "black")+
...
vindex%>%
  filter(phage != "noPHI") %>% 
  filter(dilution>=max.dilut)%>%
  group_by(phage, log.moi)%>%
  summarise( Virulence=mean(Vi), mn=min(Vi), mx=max(Vi), n=n())%>%
    ggplot(aes(log.moi-1, Virulence))+
      geom_line(size=1)+
      geom_pointrange(aes(ymin=mn, ymax=mx ), shape=21, size=.5, fill = "white")+
