...
p <- d.rax %>% 
  ...
  geom_text(aes(x=branch, label=node), color="red", size=3, alpha=0.2)

  ggsave(here("phylo","plots","sigma_nodeNUMS_rooted.pdf"), 
...
#   geom_tippoint(aes(color=group), size=2, shape=20)+
#   geom_tiplab(aes(label=bs.label), color="blue", size=3, offset = .1)+
#   geom_nodepoint(aes(fill=support), colour = "transparent", size=1, shape = 21)+
#   scale_fill_hue(h = c(0, 90),na.translate =FALSE)
...
d.rax %>% 
  ...
  as.treedata(.) %>% 
  ggtree(aes(color = clade), 
         layout = "fan", open.angle=10, size=0.1)+
  # geom_tiplab(aes(label=circ1.lab, color = clade), size=2, offset = 0.3)+
  geom_fruit(geom = "geom_tile",
             mapping=aes(y=ID, fill=clade),color = "transparent")+
  geom_fruit(geom = "geom_text",
             mapping = aes(y=ID, label=circ1.lab, color = clade), size=2, offset = -.2)+
  # geom_fruit(geom = geom_text,
  ...
  new_scale_color()+
  geom_fruit(geom = "geom_tile",
             mapping=aes(y=ID, fill=host_phylum),color = "transparent",offset = 0.2)+
  # geom_fruit(geom = geom_text,
  ...
  new_scale_fill()+
  geom_fruit(geom = "geom_tile",
             mapping=aes(y=ID, fill=tigr.type), offset = 0.2, color = "transparent")+
...
ggsave(here("phylo","plots","sigma_circle_rooted.pdf"), height=10, width = 10)
